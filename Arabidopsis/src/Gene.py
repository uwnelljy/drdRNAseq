...
class Gene:
    ...
    def get_target_chr_start_end(target_annotation):
        ...
        for va in target_annotation:
            if va._type == 'gene':
                return va._chrid, va._start, va._end, va._strand
        return None
    ...
    def get_region_gene(chrom, commblock, samples):
        ...
        for sample_block in samples:
            ...
            for start_end in commblock:
                data_without_nan = Gene.without_nan(sample_block._sample.values(chrom, start_end[0], start_end[1]))
                one_sample += data_without_nan
    ...
    def get_whole_gene(chrom, start, end, samples):
        ...
        whole_gene = []
        for sample_block in samples:
            whole_gene_with_nan = sample_block._sample.values(chrom, start, end)
            whole_gene_without_nan = Gene.without_nan(whole_gene_with_nan)
            whole_gene.append(whole_gene_without_nan)
        return whole_gene

    @staticmethod
    def without_nan(data: list):
        data_without_nan = []
        for va in data:
            if va != va:
                data_without_nan.append(0)
            else:
                data_without_nan.append(va)
